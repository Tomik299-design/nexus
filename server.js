...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', data => {
    ...
    if (msg.type === 'presence' && msg.m) {
      ...
      if (msg.m.id && msg.m.name) {
        accounts[msg.m.id] = {
          ...
          avatar: msg.m.avatar, font: msg.m.font, ts: Date.now()
        };
        saveAccounts();
      }
    ...
    if (msg.type === 'account_save' && msg.userId && msg.servers) {
      if (!accounts[msg.userId]) accounts[msg.userId] = {};
      accounts[msg.userId].servers = msg.servers;
      accounts[msg.userId].roles   = msg.roles || {};
