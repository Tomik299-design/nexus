...
// ══════════════════════════════════════════
// ══════════════════════════════════════════
//  AUTO CHANGELOG — reads from git log
// ══════════════════════════════════════════
...
function getAutoChangelog() {
  try {
    ...
    const log = execSync(
      'git log --oneline --no-merges -20 --pretty=format:"%h|%s|%cd" --date=short 2>/dev/null',
      { cwd: __dirname, timeout: 3000 }
    ).toString().trim();
    ...
    const commits = log.split('\n').map(line => {
      ...
      // Auto-detect type from commit message
      const lower = (msg || '').toLowerCase();
      let type = 'imp';
      if (lower.startsWith('fix') || lower.includes('oprav') || lower.includes('bug')) type = 'fix';
      else if (lower.startsWith('add') || lower.startsWith('feat') || lower.startsWith('new') ||
               lower.startsWith('přid') || lower.startsWith('nov')) type = 'new';
      else if (lower.startsWith('remove') || lower.startsWith('delet') || lower.startsWith('odebr')) type = 'rem';
      return { hash, text: msg || '?', date: date || '', type };
    ...
    return [{
      version: 'latest',
      date: commits[0].date || new Date().toISOString().slice(0,10),
...
// Try git first, fallback to static
const GIT_LOG = getAutoChangelog();

const CHANGELOG = GIT_LOG || [
  {
    version: '3.0',
    date: '2026-03-24',
    items: [
      { type: 'new',  text: 'Voice calls via WebSocket relay' },
      { type: 'new',  text: 'Noise Gate — filters breathing & background noise' },
      { type: 'new',  text: 'Mic & output volume control in Audio settings' },
      { type: 'new',  text: 'Language switcher CS / EN' },
      { type: 'new',  text: 'Persistent offline members & bans saved to disk' },
      { type: 'new',  text: 'DM list in sidebar, right-click DM messages' },
      { type: 'new',  text: '@mention picker, channel permissions' },
      { type: 'fix',  text: 'Messages no longer sent to wrong chat after DM' },
      { type: 'fix',  text: 'Stay on same server & channel after page refresh' },
      { type: 'fix',  text: 'Voice room correctly shows who is in the channel' },
      { type: 'imp',  text: 'Performance optimizations for gaming in background' },
    ]
  }
];
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  body{background:#070809;font-family:'Syne',system-ui,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px 16px}
  .wrap{width:min(520px,100%);display:flex;flex-direction:column;gap:14px}
  .card{background:#0d0f12;border:1px solid #1f2836;border-radius:20px;padding:36px 32px;box-shadow:0 24px 60px rgba(0,0,0,.7);animation:fadeUp .5s cubic-bezier(.22,1,.36,1) forwards;text-align:center}
  ...
  .badge{display:inline-flex;align-items:center;gap:8px;background:rgba(79,255,176,.07);border:1px solid rgba(79,255,176,.2);border-radius:20px;padding:7px 16px;font-size:13px;color:#4fffb0;margin-bottom:20px;font-weight:600}
  ...
  .progress-bar{height:100%;background:linear-gradient(90deg,#4fffb0,#7c6aff);border-radius:8px;animation:prog 12s linear infinite;width:0}
  @keyframes prog{from{width:0%}to{width:100%}}
  .refresh-note{font-size:12px;color:#3a4654}
  .refresh-note a{color:#4fffb0;cursor:pointer;text-decoration:none}
  .changelog{background:#0d0f12;border:1px solid #1f2836;border-radius:20px;padding:24px 28px;animation:fadeUp .5s cubic-bezier(.22,1,.36,1) .15s both}
  .cl-header{display:flex;align-items:center;gap:10px;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid #1f2836}
  ...
  .cl-item{display:flex;align-items:flex-start;gap:10px;padding:6px 0;border-bottom:1px solid #131720;font-size:13px;color:#94a3b8;line-height:1.5}
  .cl-item:last-child{border-bottom:none}
  .cl-badge{font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;flex-shrink:0;margin-top:1px;letter-spacing:.4px;text-transform:uppercase}
...
<div class="wrap">
  <div class="card">
    ...
    <p class="refresh-note">Auto-refresh in <strong style="color:#e2e8f0">12 seconds</strong> &nbsp;·&nbsp; <a href="javascript:location.reload()">Refresh now</a></p>
  </div>
  \${buildChangelogHtml()}
</div>
<script>
  function tryLoad(){fetch(location.href,{method:'HEAD'}).then(function(r){if(r.status===200)location.reload();else setTimeout(tryLoad,3000);}).catch(function(){setTimeout(tryLoad,3000);});}
...
const httpServer = http.createServer((req, res) => {
  ...
  if (urlPath === '/' || urlPath === '/index.html' || urlPath === '/NexusChat.html') {
    // Show loading screen during cold start
    if (!serverReady) {
