...
// Build changelog HTML (no nested template literals)
...
<style>
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Logo */
  .logo {
    font-size: 56px; margin-bottom: 32px;
    display: inline-block;
    animation: float 3s ease-in-out infinite;
    filter: drop-shadow(0 0 24px rgba(79,255,176,.4));
  }
  @keyframes float {
  ...
  .tagline { font-size: 14px; color: #3a4654; margin-bottom: 48px; }
  ...
  /* Status text */
  .status { font-size: 13px; color: #3a4654; letter-spacing: .3px; }
  .status span {
    color: #4fffb0;
  ...
  .dots i {
    ...
    background: #4fffb0; display: inline-block;
  ...
  /* Auto reload */
  .reload { font-size: 11px; color: #1f2836; margin-top: 32px; }
  .reload a { color: #2a3545; cursor: pointer; text-decoration: none; }
  .reload a:hover { color: #4fffb0; }
</style>
</head>
<body>
<div class="wrap">
  <div class="logo">⚡</div>
  <div class="appname">NexusChat</div>
  <div class="tagline">Starting up the server...</div>
  <div class="ring"></div>
  <div class="status">
...
<script>
  // Auto reload every 3 seconds until server is ready
  function tryLoad() {
    fetch(location.href, { method: 'HEAD' })
      .then(function(r) {
        if (r.status === 200) { location.reload(); }
        else { setTimeout(tryLoad, 3000); }
      })
      .catch(function() { setTimeout(tryLoad, 3000); });
  }
...
const ipBans       = {}; // ip -> { srvId -> { reason, ts } }
const history      = {};
const vcState      = {};
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', data => {
    ...
    if (msg.type === 'presence' && msg.m) {
      ...
      // Store IP → ID mapping for persistence across browsers
    ...
    // Store IP-level ban
    if (msg.type === 'ip_ban' && msg.targetId && msg.srvId) {
      // Find IP of target user
      for (const [client, info] of clients) {
        if (info.id === msg.targetId && info.ip) {
          if (!ipBans[info.ip]) ipBans[info.ip] = {};
          ipBans[info.ip][msg.srvId] = { reason: msg.reason || '', ts: Date.now() };
          break;
        }
      }
    }

    // Check if this client is banned on connect
    if (msg.type === 'check_ban' && msg.srvId) {
      const info = clients.get(ws) || {};
      const banned = info.ip && ipBans[info.ip] && ipBans[info.ip][msg.srvId];
      if (banned) {
        try { ws.send(JSON.stringify({ type: 'you_are_banned', srvId: msg.srvId, reason: banned.reason })); } catch {}
      }
    }
